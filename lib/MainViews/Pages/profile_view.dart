                  ...
                  child: Column(
                    children: [
                      ...
                      ImagePickerWidget(
                        ...
                        initialImage:provider.user.image == null? Image(image: Svg("assets/profile-logo.svg"),).image : Image.memory(userImage!).image,
                      ...
                      Text("${provider.user.firstName} ${provider.user.lastName}",textAlign: TextAlign.center,style: TextStyle(fontSize: 40),),

                      if(provider.user is Teacher && provider.user.rating != 0)
                        Rating(rating: provider.user.rating!),

                      if(provider.user is Teacher)
                        Text("${provider.user.price} ${provider.user.currency}/Session",textAlign: TextAlign.center,style: TextStyle(fontSize: 20,color: Colors.black.withOpacity(0.5)),),

                      Text("${provider.user.section} Section",textAlign: TextAlign.center,style: TextStyle(fontSize: 20,color: Colors.black.withOpacity(0.5)),),
                      ...
                      if(provider.user is Teacher)
                        FutureBuilder(
                          future: context.read<ListFeedbackViewModel>().get(provider.user.id),
                          builder: (context, snapshot) {
